...
vote_democrat <- paste0("For each voter in the voter data, is the voter likely to vote ",
                        ...
                        "one word, yes no. Rules (MUST follow all): Can only return unknown sparingly, ",
                        "do not omit any voter, Produce EXACTLY 100 predictions with the output in the ",
                        "order of voter_id's given, do not fabricate voter_id values only use those provide, ",
...
vote_republican <- paste0("For each voter in the voter data, is the voter likely to vote ",
                          ...
                          "one word, yes no. Rules (MUST follow all): Can only return unknown sparingly, do not ",
                          "omit any voter, Produce EXACTLY 100 predictions with the output in the order of voter_id's given, ",
                          "do not fabricate voter_id values only use those provide, Output format: voter_id. prediction")
vote_third_party <- paste0("For each voter in the voter data, is the voter likely to vote third party in the next U.S. ",
                           "presidential election? Respond with one word, yes no. Rules (MUST follow all): ",
                           "Can only return unknown sparingly, do not omit any voter, Produce EXACTLY 100 predictions ",
                           "with the output in the order of voter_id's given, do not fabricate voter_id values ",
                           "only use those provide, Output format: voter_id. prediction")
easy_to_predict <- paste0("For each voter in the voter data, is their behavior easy to predict? Respond with one word, ",
                          "yes no. Rules (MUST follow all): Can only return unknown sparingly, do not omit any voter, ",
                          "Produce EXACTLY 100 predictions with the output in the order of voter_id's given, ",
                          "do not fabricate voter_id values only use those provide, Output format: voter_id. prediction")
...
gov_services <- paste0("for each voter in the voter data , does the voter think A: small government providing ",
                       ...
                       "Respond with only A B. Rules (MUST follow all): Can only return unknown sparingly, ",
                       "do not omit any voter, Produce EXACTLY 100 predictions with the output in the order ",
                       "of voter_id's given, do not fabricate voter_id values only use those provide, ",
                       "Output format: voter_id. prediction")
...
candidates_view <- paste0("for each voter in the voter data, which comes closer to the voters view of candidates ",
                          ...
                          "Respond with only A B. Rules (MUST follow all): Can only return unknown sparingly, ",
                          "do not omit any voter, Produce EXACTLY 100 predictions with the output in the order of voter_id's given, ",
                          "do not fabricate voter_id values only use those provide, Output format: voter_id. prediction")
...
energy_initiative <- paste0("For each voter in the voter data, how would the voter vote on the following: ",
                            ...
                            "only return unknown sparingly, do not omit any voter, Produce EXACTLY 100 predictions ",
                            "with the output in the order of voter_id's given, do not fabricate voter_id values only use those provide, ",
                            "Output format: voter_id. prediction")
...
carbon_tax_initiative <- paste0("For each voter in the voter data, how would the voter vote on the following: ",
                                ...
                                "Should this measure be enacted into law? Respond with one word, yes no. Rules (MUST follow all): Can ",
                                "only return unknown sparingly, do not omit any voter, Produce EXACTLY 100 predictions with the ",
                                "output in the order of voter_id's given, do not fabricate voter_id values only use those provide, ",
                                "Output format: voter_id. prediction")
insurance_initiative <- paste0("For each voter in the voter data, how would the voter vote on the following: ",
                               ...
                               "Respond with one word, yes no. Rules (MUST follow all): Can only return unknown sparingly, ",
                               "do not omit any voter, Produce EXACTLY 100 predictions with the output in the order of voter_id's ",
                               "given, do not fabricate voter_id values only use those provide, Output format: voter_id. prediction")
...
supreme_court <- paste0("For each voter in the voter data, would the voter vote ",
                        ...
                        "with only the first name of the candidate. Rules (MUST follow all): Can only return unknown sparingly, ",
                        "do not omit any voter, Produce EXACTLY 100 predictions with the output in the order of voter_id's given, ",
                        "do not fabricate voter_id values only use those provide, Output format: voter_id. prediction")
...
governor <- paste0("For each voter in the voter data, would the voter vote ",
                   ...
                   "sparingly, do not omit any voter, Produce EXACTLY 100 predictions with the output in the order of ",
                   "voter_id's given, do not fabricate voter_id values only use those provide, Output format: voter_id. prediction")
state_treasurer <- paste0("For each voter in the voter data, would the voter vote ",
                          ...
                          "in Washington State, in the 2024 general election?, Respond ",
                          "with only the first name of the candidate. Rules (MUST follow all): Can only return unknown ",
                          "sparingly, do not omit any voter, Produce EXACTLY 100 predictions with the output in the order ",
                          "of voter_id's given, do not fabricate voter_id values only use those provide, ",
                          "Output format: voter_id. prediction")
attorney_general <- paste0("For each voter in the voter data, would the voter vote ",
                           ...
                           "in Washington State, in the 2024 general election?, Respond ",
                           "with only the first name of the candidate. Rules (MUST follow all): Can only return unknown ",
                           "sparingly, do not omit any voter, Produce EXACTLY 100 predictions with the output in ",
                           "the order of voter_id's given, do not fabricate voter_id values only use those provide, ",
                           "Output format: voter_id. prediction")
